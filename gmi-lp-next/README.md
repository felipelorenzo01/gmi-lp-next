...
## 7. Assets a finalizar (designer)

- **Logos das marcas e clientes** entram hoje como wordmarks de texto (`LogoStrip` em
  `app/page.tsx`). Substituir pelos SVGs finais (3M, Sherwin-Williams, Vonixx, Farben, etc.;
  Azul, Carbel, GOL, Grupo Líder, Saritur).
- **Fonte Averta**: hoje usamos *Manrope* (livre) como substituta. Se a GMI tiver licença
  web da Averta, trocar nas importações de `app/layout.tsx`.
- As imagens de produto/galpão vieram do export do InDesign (`public/img/`). Trocar por
  versões finais/recortadas quando houver.
