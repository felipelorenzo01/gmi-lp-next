...
export async function POST(req: Request) {
  ...
  // Log explícito do resultado — visível no Vercel Logs
  if (suri.skipped) {
    console.warn("[suri] não configurada — pulando envio.");
  } else if (suri.ok) {
    console.log(`[suri] ✓ contato importado | id=${suri.contactId} status=${suri.status}`);
  } else {
    console.error(`[suri] ✗ erro ao importar contato | status=${suri.status} erro=${suri.error}`);
  }
